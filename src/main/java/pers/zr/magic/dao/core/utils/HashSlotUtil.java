...
public class HashSlotUtil {
	...
	public static int getSlot(String key, int slotCount) {
		int s = key.indexOf("{");
		if (s > -1) {
		...
		// equivalent to getCRC16(key) % slotNumber
		return getCRC16(key) & (slotCount - 1);
	}

	public static int getSlot(byte[] key, int slotCount) {
		int s = -1;
		int e = -1;
		...
		if (s > -1 && e > -1 && e != s + 1) {
			return getCRC16(key, s + 1, e) & (slotCount - 1);
		}
		return getCRC16(key) & (slotCount - 1);
	}
	 ...
	 * 
	 * @param bytes
	 * @return CRC16 as integer value See <a href=
